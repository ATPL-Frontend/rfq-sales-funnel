...
export async function listRFQs(req, res) {
  try {
    ...
    const [[{ total }]] = await pool.query(
      `SELECT COUNT(*) AS total FROM rfq r JOIN customers c ON c.id = r.customer_id ${whereSql}`,
      params
    );
...
export async function updateRFQ(req, res) {
  ...
  try {
    ...
    const hasPrepared = Object.prototype.hasOwnProperty.call(req.body, "prepared_by");
    const preparedIds = hasPrepared ? normalizePreparedIds(req.body.prepared_by) : [];

    await conn.beginTransaction();

    if (updates.length) {
      params.push(id);
      await conn.query(`UPDATE rfq SET ${updates.join(", ")} WHERE id=?`, params);
    }

    if (hasPrepared) {
      await conn.query("DELETE FROM rfq_prepared_people WHERE rfq_id=?", [id]);
      if (preparedIds.length) await insertPreparedPeople(conn, id, preparedIds);
    }

    await conn.commit();

    const [rows] = await pool.query(
      `SELECT r.*, c.name AS customer_name, u1.name AS salesperson_name,
              JSON_ARRAYAGG(JSON_OBJECT('id', u.id, 'name', u.name, 'email', u.email, 'short_form', u.short_form)) AS prepared_by
       FROM rfq r
       JOIN customers c ON c.id = r.customer_id
       JOIN users u1 ON u1.id = r.salesperson_id
